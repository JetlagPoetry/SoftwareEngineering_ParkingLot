package com.nku.cs.parkinglot.widget.datechoosewheelviewdialog;
...
import com.nku.cs.parkinglot.widget.datechoosewheelviewdialog.adapters.WheelViewAdapter;
import com.nku.cs.parkinglot.widget.datechoosewheelviewdialog.listeners.OnWheelChangedListener;
import com.nku.cs.parkinglot.widget.datechoosewheelviewdialog.listeners.OnWheelScrollListener;
import com.nku.cs.parkinglot.widget.datechoosewheelviewdialog.listeners.WheelView;
import com.nku.cs.parkinglot.widget.datechoosewheelviewdialog.adapters.AbstractWheelTextAdapter;
...
public class DateChooseWheelViewDialog extends Dialog implements View.OnClickListener {
    ...
    private boolean mBlnYearMonthDayGone = false; // 自定义: 年月日是否显示
    ...
    public DateChooseWheelViewDialog(Context context, DateChooseInterface dateChooseInterface) {
        ...
        mDialog = new Dialog(context,com.nku.cs.parkinglot.R.style.dialog);
        initView();
        initData();
    }
...
//                mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
    ...
    private void initView() {
        View view = LayoutInflater.from(mContext).inflate(com.nku.cs.parkinglot.R.layout.dialog_date_choose, null);
        mDialog.setContentView(view);
        mYearWheelView = (WheelView) view.findViewById(com.nku.cs.parkinglot.R.id.year_wv);
        mDateWheelView = (WheelView) view.findViewById(com.nku.cs.parkinglot.R.id.date_wv);
        mHourWheelView = (WheelView) view.findViewById(com.nku.cs.parkinglot.R.id.hour_wv);
        mMinuteWheelView = (WheelView) view.findViewById(com.nku.cs.parkinglot.R.id.minute_wv);
        mTitleTextView = (TextView) view.findViewById(com.nku.cs.parkinglot.R.id.title_tv);
        mSureButton = (Button) view.findViewById(com.nku.cs.parkinglot.R.id.sure_btn);
        mCloseDialog = (Button) view.findViewById(com.nku.cs.parkinglot.R.id.date_choose_close_btn);
        mLongTermLayout = (LinearLayout) view.findViewById(com.nku.cs.parkinglot.R.id.long_term_layout);
        mLongTermTextView = (TextView) view.findViewById(com.nku.cs.parkinglot.R.id.long_term_tv);
    ...
    // 自定义
    public void setYearMonthDayPickerGone(boolean isGone) {
        mBlnYearMonthDayGone = isGone;
        if (isGone) {
            LinearLayout.LayoutParams hourParams = new LinearLayout.LayoutParams( LinearLayout.LayoutParams.WRAP_CONTENT,
                    LinearLayout.LayoutParams.WRAP_CONTENT);
            hourParams.rightMargin = 0;

            LinearLayout.LayoutParams minuteParams = new LinearLayout.LayoutParams( LinearLayout.LayoutParams.WRAP_CONTENT,
                    LinearLayout.LayoutParams.WRAP_CONTENT);
            minuteParams.rightMargin = 0;

            mHourWheelView.setLayoutParams(hourParams);
            mMinuteWheelView.setLayoutParams(minuteParams);
    ...
    public void setTextViewStyle(String curriteItemText, CalendarTextAdapter adapter) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (curriteItemText.equals(currentText)) {
                ...
                textvew.setTextColor(mContext.getResources().getColor(com.nku.cs.parkinglot.R.color.text_10));
            } else {
                textvew.setTextSize(MIN_TEXT_SIZE);
                textvew.setTextColor(mContext.getResources().getColor(com.nku.cs.parkinglot.R.color.text_11));
            }
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case com.nku.cs.parkinglot.R.id.sure_btn://确定选择按钮监听
                ...
                dismissDialog();
                break;
            case com.nku.cs.parkinglot.R.id.date_choose_close_btn://关闭日期选择对话框
                dismissDialog();
                break;
            case com.nku.cs.parkinglot.R.id.long_term_tv://选择长期时间监听
                if (!mBlnBeLongTerm) {
                    mLongTermTextView.setBackgroundResource(com.nku.cs.parkinglot.R.drawable.gouxuanok);
                    mBlnBeLongTerm = true;
                } else {
                    mLongTermTextView.setBackgroundResource(com.nku.cs.parkinglot.R.drawable.gouxuanno);
    ...
    private class CalendarTextAdapter extends AbstractWheelTextAdapter {
        ...
        protected CalendarTextAdapter(Context context, ArrayList<String> list, int currentItem, int maxsize, int minsize) {
            super(context, com.nku.cs.parkinglot.R.layout.item_birth_year, com.nku.cs.parkinglot.R.id.tempValue, currentItem, maxsize, minsize);
            this.list = list;
        }
